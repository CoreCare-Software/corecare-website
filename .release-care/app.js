...
let medicationData={medications:[],administrations:[],stockTransactions:[]},dailyMarData={rows:[]};
...
function medicationSelectedClient(){return $('#medication-client')?.value||'';}
function medicationSelectedDate(){return $('#mar-date')?.value||new Date().toISOString().slice(0,10);}
async function loadMedicationModule(){if(!clients.length)await loadClients();populateMedicationClients();const date=$('#mar-date');if(date&&!date.value)date.value=new Date().toISOString().slice(0,10);const id=medicationSelectedClient();if(id)await loadMedicationForClient(id);else renderMedication();}
async function loadMedicationForClient(clientId){const date=medicationSelectedDate();[medicationData,dailyMarData]=await Promise.all([api('/api/medication?clientId='+encodeURIComponent(clientId)),api('/api/medication/daily-mar?clientId='+encodeURIComponent(clientId)+'&date='+encodeURIComponent(date))]);renderMedication();}
function medicationOutcomeClass(outcome){return ['administered','prompted'].includes(outcome)?'success':['refused','omitted','missed','unavailable'].includes(outcome)?'danger':'neutral';}
function renderMedication(){const list=$('#medication-list'),mar=$('#mar-list'),summary=$('#medication-summary'),daily=$('#daily-mar-list'),stock=$('#medication-stock'),alerts=$('#medication-alerts');if(!list)return;const meds=medicationData.medications||[],entries=medicationData.administrations||[],transactions=medicationData.stockTransactions||[],rows=dailyMarData?.rows||[];const low=meds.filter(x=>x.stock_quantity!==null&&Number(x.stock_quantity)<=Number(x.low_stock_threshold??5));const due=rows.filter(x=>x.kind==='scheduled'&&!x.entry);if(summary)summary.innerHTML=[['Active medicines',meds.filter(x=>x.status==='active').length],['Due / outstanding',due.length],['PRN medicines',meds.filter(x=>x.is_prn).length],['Low stock',low.length]].map(x=>`<article><span>${x[0]}</span><strong>${x[1]}</strong></article>`).join('');if(alerts)alerts.innerHTML=[...low.map(m=>`<div class="notice-banner warning"><div><strong>Low stock: ${escapeHtml(m.name)}</strong><span>${escapeHtml(String(m.stock_quantity))} ${escapeHtml(m.stock_unit||'units')} remaining; threshold ${escapeHtml(String(m.low_stock_threshold??5))}.</span></div></div>`),...due.filter(r=>new Date(r.scheduledAt)<new Date()).map(r=>`<div class="notice-banner danger"><div><strong>Outstanding dose: ${escapeHtml(r.medication.name)}</strong><span>${new Date(r.scheduledAt).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})} dose has no recorded outcome.</span></div></div>`)].join('');
const caption=$('#daily-mar-caption');if(caption)caption.textContent=medicationSelectedClient()?new Date(medicationSelectedDate()+'T12:00:00').toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long',year:'numeric'}):'Choose a client and date.';
if(daily)daily.innerHTML=rows.length?rows.map(r=>{const m=r.medication,e=r.entry;const when=r.kind==='scheduled'?new Date(r.scheduledAt).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'}):(r.kind==='prn'?'PRN':'No fixed time');return `<article class="daily-mar-row ${e?'completed':''}"><div class="mar-time"><strong>${escapeHtml(when)}</strong><small>${r.kind==='prn'?'When required':'Scheduled'}</small></div><div class="mar-medicine"><strong>${escapeHtml(m.name)} ${escapeHtml(m.strength||'')}</strong><span>${escapeHtml(m.dose)} · ${escapeHtml(m.route||'Route not recorded')}</span><small>${escapeHtml(m.instructions||m.prn_protocol||'')}</small></div><div class="mar-status">${e?`<span class="badge ${medicationOutcomeClass(e.outcome)}">${escapeHtml(e.outcome)}</span><small>${escapeHtml(e.recorded_by_name||'Recorded user')} · ${new Date(e.administered_at).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}</small>`:`<span class="badge warning">Outstanding</span>`}</div><div class="record-actions">${e?`<button class="secondary-button compact" data-correct-mar="${escapeHtml(e.id)}">Correct</button>`:`<button class="primary-button compact" data-administer-medication="${escapeHtml(m.id)}" data-scheduled-at="${escapeHtml(r.scheduledAt||'')}">Record outcome</button>`}</div></article>`}).join(''):'<p class="muted">No active scheduled medication for this date.</p>';
list.innerHTML=meds.length?meds.map(m=>`<article class="record-card medication-card"><div class="record-card-heading"><div><span class="badge ${m.status==='active'?'active':'neutral'}">${escapeHtml(m.status)}</span><h3>${escapeHtml(m.name)} ${escapeHtml(m.strength||'')}</h3></div><span class="badge ${m.is_prn?'warning':'neutral'}">${m.is_prn?'PRN':'Regular'}</span></div><p><strong>${escapeHtml(m.dose)}</strong> · ${escapeHtml(m.route||'Route not recorded')}</p><p class="muted">${escapeHtml((m.scheduledTimes||[]).join(', ')||m.frequency||'No schedule recorded')}</p><p>${escapeHtml(m.instructions||'')}</p><p><b>Stock:</b> ${m.stock_quantity===null?'Not tracked':escapeHtml(String(m.stock_quantity))+' '+escapeHtml(m.stock_unit||'')} ${m.stock_quantity!==null&&Number(m.stock_quantity)<=Number(m.low_stock_threshold??5)?'<span class="badge danger">Low</span>':''}</p>${m.is_prn&&m.prn_protocol?`<div class="notice-banner small-notice"><div><strong>PRN protocol</strong><span>${escapeHtml(m.prn_protocol)}</span></div></div>`:''}<div class="record-actions"><button class="primary-button compact" data-administer-medication="${escapeHtml(m.id)}">Record administration</button><button class="secondary-button compact" data-stock-medication="${escapeHtml(m.id)}">Adjust stock</button><button class="secondary-button compact" data-edit-medication="${escapeHtml(m.id)}">Edit</button></div></article>`).join(''):'<p class="muted">No medications recorded for this client.</p>';
if(stock)stock.innerHTML=meds.length?`<div class="stock-balance-grid">${meds.map(m=>`<article><div><strong>${escapeHtml(m.name)} ${escapeHtml(m.strength||'')}</strong><span>${m.stock_quantity===null?'Not tracked':escapeHtml(String(m.stock_quantity))+' '+escapeHtml(m.stock_unit||'')}</span></div><button class="secondary-button compact" data-stock-medication="${escapeHtml(m.id)}">Adjust</button></article>`).join('')}</div><div class="stock-history"><h3>Recent stock transactions</h3>${transactions.length?transactions.map(t=>`<div class="stock-transaction"><div><strong>${escapeHtml(t.medication_name)}</strong><small>${new Date(t.created_at).toLocaleString('en-GB')} · ${escapeHtml(t.recorded_by_name||'Recorded user')}</small></div><span class="${Number(t.quantity)>=0?'positive':'negative'}">${Number(t.quantity)>=0?'+':''}${escapeHtml(String(t.quantity))}</span><small>${escapeHtml(t.reason||t.transaction_type)}</small></div>`).join(''):'<p class="muted">No stock transactions recorded.</p>'}</div>`:'<p class="muted">No medications recorded.</p>';
mar.innerHTML=entries.length?entries.map(e=>`<div class="mar-entry ${e.is_void?'void-entry':''}"><div><strong>${escapeHtml(e.medication_name)}</strong><small>${new Date(e.administered_at).toLocaleString('en-GB')}${e.scheduled_at?' · due '+new Date(e.scheduled_at).toLocaleString('en-GB'):''}</small></div><div><span class="badge ${medicationOutcomeClass(e.outcome)}">${escapeHtml(e.outcome)}</span><small>${escapeHtml(e.reason||e.notes||'No additional notes')}</small>${e.prn_effectiveness?`<small><b>PRN effect:</b> ${escapeHtml(e.prn_effectiveness)}</small>`:''}${e.is_void?`<small><b>Corrected:</b> ${escapeHtml(e.correction_reason||'Audited correction')}</small>`:''}</div><div><small>${escapeHtml(e.recorded_by_name||'Recorded user')}</small>${!e.is_void?`<button class="secondary-button compact" data-correct-mar="${escapeHtml(e.id)}">Correct</button>`:''}</div></div>`).join(''):'<p class="muted">No MAR entries recorded.</p>';bindMedicationActions();}
function bindMedicationActions(){$$('[data-administer-medication]').forEach(b=>b.onclick=()=>openAdministration(b.dataset.administerMedication,b.dataset.scheduledAt||''));$$('[data-edit-medication]').forEach(b=>b.onclick=()=>openMedicationDialog((medicationData.medications||[]).find(x=>x.id===b.dataset.editMedication)));$$('[data-stock-medication]').forEach(b=>b.onclick=()=>openStockDialog(b.dataset.stockMedication));$$('[data-correct-mar]').forEach(b=>b.onclick=()=>openCorrectionDialog(b.dataset.correctMar));}
function openMedicationDialog(m=null){const clientId=medicationSelectedClient();if(!clientId)return showToastError(new Error('Choose a client first.'));const f=$('#medication-form');f.reset();const error=$('#medication-error');if(error){error.hidden=true;error.textContent='';}initialiseMedicationCatalogue();f.elements.id.value=m?.id||'';for(const k of ['name','strength','form','route','dose','frequency','startDate','endDate','stockQuantity','stockUnit','lowStockThreshold','status','instructions','prnProtocol','minIntervalMinutes','maxDose24h','discontinuedReason'])if(f.elements[k]&&m)f.elements[k].value=m[k.replace(/[A-Z]/g,x=>'_'+x.toLowerCase())]??m[k]??'';f.elements.scheduledTimes.value=(m?.scheduledTimes||[]).join(', ');f.elements.isPrn.checked=!!m?.is_prn;$('#medication-dialog').showModal();setTimeout(()=>f.elements.name?.focus(),50);}
function openAdministration(id,scheduledAt=''){const m=(medicationData.medications||[]).find(x=>x.id===id),f=$('#administration-form');f.reset();f.elements.medicationId.value=id;f.elements.scheduledAt.value=scheduledAt;f.elements.doseGiven.value=m?.dose||'';f.elements.administeredAt.value=new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,16);f.querySelectorAll('.prn-only').forEach(x=>x.hidden=!m?.is_prn);$('#administration-title').textContent='Record '+(m?.name||'medication');$('#administration-dialog').showModal();}
function openStockDialog(id){const m=(medicationData.medications||[]).find(x=>x.id===id),f=$('#stock-form');f.reset();f.elements.medicationId.value=id;$('#stock-title').textContent='Adjust stock · '+(m?.name||'Medication');$('#stock-dialog').showModal();}
function openCorrectionDialog(id){const e=(medicationData.administrations||[]).find(x=>x.id===id),f=$('#correction-form');f.reset();f.elements.administrationId.value=id;f.elements.outcome.value=e?.outcome||'administered';f.elements.doseGiven.value=e?.dose_given||'';f.elements.stockUsed.value=Math.abs(Number(e?.stock_change||0))||'';f.elements.administeredAt.value=e?.administered_at?new Date(new Date(e.administered_at).getTime()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,16):'';$('#correction-dialog').showModal();}
...
$('#medication-client')?.addEventListener('change',e=>e.target.value?loadMedicationForClient(e.target.value).catch(showToastError):(medicationData={medications:[],administrations:[],stockTransactions:[]},dailyMarData={rows:[]},renderMedication()));
$('#mar-date')?.addEventListener('change',()=>medicationSelectedClient()&&loadMedicationForClient(medicationSelectedClient()).catch(showToastError));
$('#add-medication')?.addEventListener('click',()=>openMedicationDialog());
$$('[data-medication-tab]').forEach(b=>b.addEventListener('click',()=>{$$('[data-medication-tab]').forEach(x=>x.classList.toggle('active',x===b));$$('[data-medication-panel]').forEach(x=>x.hidden=x.dataset.medicationPanel!==b.dataset.medicationTab);}));
$('#print-mar')?.addEventListener('click',()=>window.print());
$('#medication-form')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,el=$('#medication-error'),submit=f.querySelector('[type=submit]');if(el){el.hidden=true;el.textContent='';}const d=Object.fromEntries(new FormData(f));d.clientId=medicationSelectedClient();d.isPrn=f.elements.isPrn.checked;d.scheduledTimes=String(d.scheduledTimes||'').split(',').map(x=>x.trim()).filter(Boolean);try{if(submit)submit.disabled=true;await api('/api/medication',{method:'POST',body:JSON.stringify(d)});$('#medication-dialog')?.close();await loadMedicationForClient(d.clientId)}catch(err){if(el){el.textContent=err.message||'Medication could not be saved.';el.hidden=false;}else showToastError(err);}finally{if(submit)submit.disabled=false;}});
$('#administration-form')?.addEventListener('submit',async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget));try{await api('/api/medication/'+encodeURIComponent(d.medicationId)+'/administer',{method:'POST',body:JSON.stringify(d)});$('#administration-dialog').close();await loadMedicationForClient(medicationSelectedClient());}catch(err){const el=$('#administration-error');el.textContent=err.message;el.hidden=false;}});
$('#stock-form')?.addEventListener('submit',async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget));try{await api('/api/medication/'+encodeURIComponent(d.medicationId)+'/stock',{method:'POST',body:JSON.stringify(d)});$('#stock-dialog').close();await loadMedicationForClient(medicationSelectedClient());}catch(err){const el=$('#stock-error');el.textContent=err.message;el.hidden=false;}});
$('#correction-form')?.addEventListener('submit',async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget));try{await api('/api/medication/administrations/'+encodeURIComponent(d.administrationId)+'/correct',{method:'POST',body:JSON.stringify(d)});$('#correction-dialog').close();await loadMedicationForClient(medicationSelectedClient());}catch(err){const el=$('#correction-error');el.textContent=err.message;el.hidden=false;}});
