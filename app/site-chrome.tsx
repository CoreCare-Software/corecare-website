  ...
  return <>
    ...
    <header className="site-header"><div className="site-shell nav-shell">
      ...
      <nav id="main-navigation" className={mobileOpen ? "open" : ""} aria-label="Main navigation">
        ...
        <Link href="/demos" onClick={() => setMobileOpen(false)}>Demonstrations</Link>
...
export function SiteFooter() {
  return <footer><div className="site-shell footer-grid"><div className="footer-brand"><Brand inverse /><p>One family of focused, practical business software.</p><address>{COMPANY_ADDRESS}</address></div><div><strong>Products</strong>{CUSTOMER_PRODUCTS.map((product) => <Link key={product.code} href={`/products/${product.slug}`}>{product.shortName}</Link>)}<Link href="/product-status">Product status</Link></div><div><strong>Get started</strong><Link href="/solutions">Solutions by sector</Link><Link href="/demos">Interactive product demos</Link><Link href="/trial">30-day free trial</Link><Link href="/plans">Plans and limits</Link><Link href="/login">Customer login</Link><Link href="/account-help">Account help</Link><Link href="/about">About CoreCare</Link><Link href="/status">Service status</Link><Link href="/contact">Book a demonstration</Link></div><div><strong>Trust and legal</strong><Link href="/compliance">Data protection</Link><Link href="/data-rights">Your data rights</Link><Link href="/data-processing-agreement">Customer DPA</Link><Link href="/customer-terms">Customer terms</Link><Link href="/data-retention">Retention</Link><Link href="/subprocessors">Subprocessors</Link><Link href="/security">Security</Link><Link href="/privacy">Privacy</Link><Link href="/cookies">Cookies</Link><Link href="/terms">Website terms</Link><Link href="/legal">Legal information</Link><a href={`tel:${COMPANY_DETAILS.telephoneHref}`}>{COMPANY_DETAILS.telephoneDisplay}</a><a href="mailto:hello@corecaresystems.co.uk">hello@corecaresystems.co.uk</a></div></div><div className="site-shell footer-bottom"><span>© {new Date().getFullYear()} {COMPANY_DETAILS.proprietorName}, trading as {COMPANY_DETAILS.legalName}</span><span>Built for better working days.</span></div></footer>;
}
