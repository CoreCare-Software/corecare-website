...
type LoginTarget =
  | (Pick<CoreCareProduct, "code" | "name" | "liveUrl"> & { handoffOrigin?: string })
  | { code: "PLATFORM"; name: string; liveUrl: string; handoffOrigin: string };
...
function customerTarget(product: CoreCareProduct): LoginTarget {
  return { code: product.code, name: product.name, liveUrl: productOrigin(product.code, product.liveUrl) };
}

function ownerTarget(email: string): LoginTarget | null {
  const runtime = env as unknown as Record<string, string | undefined>;
  const allowedEmails = new Set(clean(runtime.CORECARE_OWNER_EMAILS, 1000).split(",").map((value) => value.trim().toLowerCase()).filter(Boolean));
  if (!allowedEmails.has(email)) return null;
  return {
    code: "PLATFORM",
    name: "CoreCare Administration",
    liveUrl: (clean(runtime.CORECARE_PLATFORM_ORIGIN, 500) || "https://corecare-platform.cselectricalservices11.workers.dev").replace(/\/$/, ""),
    handoffOrigin: (clean(runtime.CORECARE_PLATFORM_HANDOFF_ORIGIN, 500) || "https://owner.corecaresystems.co.uk").replace(/\/$/, ""),
  };
}

async function checkCredentials(product: LoginTarget, email: string, password: string) {
  const origin = product.liveUrl.replace(/\/$/, "");
  const handoffOrigin = (product.handoffOrigin || origin).replace(/\/$/, "");
  try {
    ...
    return { valid: true as const, status: 200, origin, handoffOrigin };
  } catch {
    return { valid: false as const, status: 502, message: `${product.name} could not be reached.`, origin, handoffOrigin };
  }
}
...
export async function POST(request: Request) {
  ...
  if (selected) {
    const target = customerTarget(selected);
    const result = await checkCredentials(target, email, password);
    ...
    return Response.json({ ok: true, product: { code: selected.code, name: selected.name }, handoffUrl: `${result.handoffOrigin}/api/auth/portal-login` }, { headers: { "cache-control": "no-store" } });
  }

  const targets: LoginTarget[] = PRODUCTS.map(customerTarget);
  const owner = ownerTarget(email);
  if (owner) targets.push(owner);
  const results = await Promise.all(targets.map(async (product) => ({ product, result: await checkCredentials(product, email, password) })));
  ...
  if (matches.length === 1) {
    ...
    return Response.json({ ok: true, product: { code: match.product.code, name: match.product.name }, handoffUrl: `${match.result.handoffOrigin}/api/auth/portal-login` }, { headers: { "cache-control": "no-store" } });
  }
  if (matches.length > 1) return Response.json({
    ...
    products: matches.map(({ product, result }) => ({
      code: product.code,
      name: product.name,
      handoffUrl: `${result.handoffOrigin}/api/auth/portal-login`,
    })),
