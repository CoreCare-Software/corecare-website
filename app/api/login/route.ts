...
export async function POST(request: Request) {
  ...
  if (matches.length === 1) {
    ...
    return Response.json({ ok: true, product: { code: match.product.code, name: match.product.name }, handoffUrl: `${match.result.origin}/api/auth/portal-login` }, { headers: { "cache-control": "no-store" } });
  }
