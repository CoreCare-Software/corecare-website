...
export async function POST(request: Request) {
  ...
  try {
    ...
    const row = await findTrialByStatusToken(token);
    ...
    const automationToken = await automationTokenForTrial(row);
    if (automationToken) {
      try {
        const dispatched = await dispatchAutomation("trial-billing-status", { automationToken, checkoutSessionId: row.checkoutSessionId || undefined });
