...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({ trial: { companyName: row.companyName, status, startsAt: row.trialStartedAt || null, endsAt: row.trialEndsAt || null, daysRemaining, provisioningStatus: row.provisioningStatus }, product: product ? { code: product.code, name: product.name, liveUrl: row.workspaceUrl || product.liveUrl } : null });
