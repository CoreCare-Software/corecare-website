      ...
      <div className="auth-form-wrap">
        ...
        {!data ? <h1>Loading your trial…</h1> : data.error ? <><h1>We couldn’t find that trial.</h1><p>{data.error}</p><Link className="button" href="/trial">Start a new trial</Link></> : <>
          ...
          <div className="trial-success-card">
            ...
            {data.trial?.needsPassword ? <form method="post" action="/api/trials/password" onSubmit={activate}>
              <input type="hidden" name="token" value={token} />
              <label className="form-label">Password<input type="password" name="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} minLength={12} maxLength={128} required /></label>
              <label className="form-label">Confirm password<input type="password" name="confirmPassword" autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} minLength={12} maxLength={128} required /></label>
