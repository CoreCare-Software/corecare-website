...
export default function DataRightsForm() {
  ...
  return <form className="contact-form" method="post" action="/api/privacy/requests" onSubmit={submit}>
    ...
    <div className="trial-form-grid">
      ...
      <label className="form-label">Product, if relevant<select name="productCode" defaultValue=""><option value="">Website or not sure</option>{CUSTOMER_PRODUCTS.map((product) => <option key={product.code} value={product.code}>{product.name}</option>)}</select></label>
